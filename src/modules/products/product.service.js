...
export const makeProductService = () => {
    ...
    const create = async ({ name, price, userId }) => {
        // validação crítica duplicada aqui por segurança
        if (!name || typeof price !== 'number' || price < 0) {
            throw new HttpError('Invalid product payload', 400, 'BAD_REQUEST');
        }
    ...
    const get = async (id) => {
        ...
        if (!found) throw new HttpError('Product not found', 404, 'NOT_FOUND');
        return found;
    };

    const patch = async (id, data) => {
        const updated = await repo.update(id, data);
        if (!updated) throw new HttpError('Product not found', 404, 'NOT_FOUND');
        return updated;
    };

    const remove = async (id) => {
        const ok = await repo.remove(id);
        if (!ok) throw new HttpError('Product not found', 404, 'NOT_FOUND');
    };
